...
class AirPlaneSeating
  ...
  def validate_input(args)
    ...
    return "The sub-arrays are in [x,y] format but 'x' and 'y' should be NON-ZERO values!" if @input_seats.any? { |x| x.include?(0) }
    return "The sub-arrays are in [x,y] format but 'x' and 'y' should be Positive values!" if @input_seats.flatten.any? { |x| x.to_i.negative? }
    begin
        ...
        elem_array.each_with_object([]).with_index do |(basic_elem_array, update_arr), index|
          update_arr << if basic_elem_array.nil?
            ...
            if index == 0
              ...
              basic_elem_array[-1] = @passengers_allocated <= @passengers_count ? @passengers_allocated.to_s.rjust(@max_seats.to_s.size, "0") : '-'*@max_seats.to_s.size
            elsif index == elem_array.size - 1
              unless basic_elem_array.size == 1
                @passengers_allocated += 1
                basic_elem_array[0] = @passengers_allocated <= @passengers_count ? @passengers_allocated.to_s.rjust(@max_seats.to_s.size, "0") : '-'*@max_seats.to_s.size
              end
            else
              @passengers_allocated += 1
              basic_elem_array[0] = @passengers_allocated <= @passengers_count ? @passengers_allocated.to_s.rjust(@max_seats.to_s.size, "0") : '-'*@max_seats.to_s.size
          ...
          update_arr << if basic_elem_array.nil?
            ...
            if basic_elem_array.size > 2
              (1..basic_elem_array.size - 2).each do |x|
                @passengers_allocated += 1
                basic_elem_array[x] = @passengers_allocated <= @passengers_count ? @passengers_allocated.to_s.rjust(@max_seats.to_s.size, "0") : '-'*@max_seats.to_s.size
              end
            end
            basic_elem_array
          end
...
seating = AirPlaneSeating.new(lines)

# puts "Enter 2D array: "
# array_ip = gets.chomp
# puts "Enter the number of passengers: "
# pass_count = gets.chomp
# seating = AirPlaneSeating.new(array_ip, pass_count)
